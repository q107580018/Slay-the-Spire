...
- 当前原版 1 代遗物目录以 `content/relics/` 为内容真源维护，并持续按本地原版资料校对；运行时效果仍按批次逐步补齐，当前已实装 `strawberry`、`pear`、`mango`、`leeches_waffle`、`old_coin` 的拿取时效果，以及 `anchor`、`bag_of_marbles`、`bag_of_preparation`、`lantern`、`clockwork_souvenir`、`thread_and_needle`、`twisted_funnel`、`ninja_scroll` 的战斗开局效果
