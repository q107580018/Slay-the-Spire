...
- 当前 session / 奖励菜单也已对齐统一奖励协议：`select_reward` 可稳定处理移除、升级、转换、复制、跳过等奖励动作，重复或未知奖励不会让领取流程崩溃，菜单文案保持中文一致
- 当前随机遗物池（战后 / Boss / Neow）已统一复用同一套 placeholder 过滤 helper；若固定奖励命中未实现或未知的遗物 / 药水 / 卡牌，`apply_reward` 会安全降级为 no-op，菜单会明确显示“未实现/不可用”
