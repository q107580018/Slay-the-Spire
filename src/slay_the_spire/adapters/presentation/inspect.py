...
def _effect_description(
    effect: Mapping[str, object], registry: ContentProviderPort
) -> str:
...
def _full_effect_summary(card_def: CardDef, registry: ContentProviderPort) -> str:
    ...
    return "；".join(
        _effect_description(effect, registry) for effect in card_def.effects
    )
...
def format_card_detail_lines(
    ...
    *,
    ...
    cost_label = (
        "无法打出" if not card_def.playable else format_card_cost(card_def.cost)
    )
    lines = [
    ...
    if card_def.upgrades_to is not None:
        ...
        lines.append(
            Text.assemble(("升级为 ", "summary.label"), render_card_name(upgraded))
        )
    return lines
...
def format_relic_detail_lines(
    ...
    disabled_action_labels = [
        disabled_actions.get(action, action) for action in relic_def.disabled_actions
    ]
    lines.append(
        Text.assemble(
            ("禁用操作 ", "summary.label"),
    ...
    if relic_def.trigger_hooks:
        lines.append(
            Text.assemble(
                ("触发 ", "summary.label"),
                summarize_trigger_hooks(relic_def.trigger_hooks),
            )
        )
    return lines
...
def format_potion_detail_lines(
    potion_id: str, registry: ContentProviderPort
) -> list[Text]:
    ...
    return [
        ...
        Text.assemble(
            ("时机 ", "summary.label"), potion_timing_label(potion_def.timing)
        ),
...
def format_reward_detail_lines(
    reward_id: str, registry: ContentProviderPort
) -> list[Text]:
    ...
    if reward_id.startswith("card_offer:") or reward_id.startswith("card:"):
        ...
        lines.append(
            Text.assemble(
                ("效果: ", "summary.label"), summarize_card_definition(card_def)
            )
        )
        return lines
    if reward_id.startswith("relic:"):
        ...
        disabled_action_labels = [
            disabled_actions.get(action, action)
            for action in relic_def.disabled_actions
        ]
        lines.append(
            Text.assemble(
                ("禁用操作: ", "summary.label"),
    ...
    if reward_id.startswith("event:"):
        ...
        lines.append(
            Text.assemble(("奖励类型: ", "summary.label"), _event_reward_label(result))
        )
        return lines
...
def render_reward_detail_panel(reward_id: str, registry: ContentProviderPort) -> Panel:
    return Panel(
        Group(*format_reward_detail_lines(reward_id, registry)),
        title="奖励详情",
        box=PANEL_BOX,
        expand=False,
    )
        ...
        for index, card_instance_id in enumerate(card_instance_ids, start=1):
            ...
            lines.append(
                Text.assemble(
                    f"{index}. ",
                    render_card_name(card_def),
                    f" | 费用 {_card_cost_label(card_def)} | 类型 {_card_type_label(card_def)} | {effect_summary}",
                )
            )
...
def render_shared_stats_panel(
    *,
    ...
    return Panel(
        Group(*[Text(line) for line in lines]), title=title, box=PANEL_BOX, expand=False
    )


def render_shared_relics_panel(
    *, title: str, run_state: RunState, registry: ContentProviderPort
) -> Panel:
    lines = ["当前遗物:"]
...
def render_shared_potions_panel(
    *, title: str, run_state: RunState, registry: ContentProviderPort
) -> Panel:
    lines = ["当前药水:"]
...
def render_relic_detail_panel(relic_id: str, registry: ContentProviderPort) -> Panel:
    return Panel(
        Group(*format_relic_detail_lines(relic_id, registry)),
        title="遗物详情",
        box=PANEL_BOX,
        expand=False,
    )
...
def _move_summary(move: Mapping[str, object], registry: ContentProviderPort) -> str:
    ...
    if isinstance(effects, list) and effects:
        return " / ".join(
            _effect_description(effect, registry)
...
def _enemy_status_label(enemy: EnemyState) -> str:
    return (
        ...
        else "".join(render_statuses(enemy.statuses).plain.splitlines())
    )


def format_enemy_list_menu(
    enemies: list[EnemyState], registry: ContentProviderPort
) -> list[str]:
    lines: list[str] = []
    if not enemies:
        lines.append("-")
...
def render_enemy_list_panel(
    ...
    for index, enemy in enumerate(combat_state.enemies, start=1):
        ...
        line.append(
            f" | 当前意图: {_move_summary(preview_enemy_move_for_display(combat_state, enemy, enemy_def) or {}, registry)}"
        )
        lines.append(line)
...
def render_enemy_detail_panel(
    combat_state: CombatState, enemy: EnemyState, registry: ContentProviderPort
) -> Panel:
    enemy_def = registry.enemies().get(enemy.enemy_id)
