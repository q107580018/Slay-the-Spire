...
- 这是一个 Python 3.12 的终端版《Slay the Spire》原型项目，界面依赖 `rich`，并提供基于 `textual` 的可选 TUI 视图。
...
- `src/slay_the_spire/app/cli.py`：CLI 入口，支持 `new` / `load`，并可通过 `--ui` 在 `terminal` 和 `textual` 间切换。
- `src/slay_the_spire/app/session.py`：会话状态、菜单路由、默认内容路径、默认存档路径。
- `src/slay_the_spire/adapters/terminal/`：Rich 终端渲染、菜单 prompt、屏幕布局。
- `src/slay_the_spire/adapters/textual/`：Textual TUI 入口、地图组件和交互式菜单面板。
...
- 当前只有 1 个角色：`ironclad`。
- 当前已有 2 幕地图：`act1`、`act2`。
- 当前地图不是固定单路径，按 `content/acts/*.json` 的 `map_config` 生成分支地图。
- 当前普通敌人池来自 `content/enemies/act1_basic.json`：`slime`、`jaw_worm`。
- 当前精英池来自 `content/enemies/act1_elites.json`：只有 `lagavulin`。
- `act1_map.json` 的 `boss_pool_id` 指向 `act1_bosses`，当前 `act1` Boss 内容已独立于精英池。
