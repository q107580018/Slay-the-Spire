...
Date: 2026-03-26
Status: Active TODO priority baseline
...
当前项目的主要短板不是 inspect，而是主流程覆盖仍然偏薄：

- 地图主循环还不够完整
- `boss_pool_id` 仍复用精英池，缺少真正的 Boss 体验
- 内容池太小，重复游玩差异不足
...
- [ ] 1.1 接通地图中实际可进入的 `shop / rest` 路径，确保不只是存在独立 use case
- [ ] 1.2 补全房间进入后到离开的状态流转，包括返回地图和下一节点选择
- [ ] 1.3 补齐奖励房和 boss 后收尾逻辑，避免“打完了但流程没有正式结束”
- [ ] 1.4 对终局页、失败页、胜利页做一轮一致性检查，保证菜单、提示和恢复逻辑稳定
...
- [ ] 2.1 新增独立 Boss content，而不是继续复用 `act1_elites`
- [ ] 2.2 为 Boss 增加与普通精英明显不同的招式表与节奏
...
更具体地说，优先把地图中 `shop / rest / reward / boss terminal` 的实际流转补完整，并以 e2e 冒烟测试作为主要验收锚点。
