...
def test_render_room_from_presentation_package() -> None:
    ...
    room_state = replace(
        session.room_state, is_resolved=True, rewards=["card_offer:anger"]
    )
    output = render_room(
...
def test_render_session_renderable_supports_opening_target_menu() -> None:
    ...
    offer = opening_flow._build_offer(
        "remove_card", "tradeoff", "remove_card", provider, Random(0)
    )
    session = replace(
        session,
...
def test_render_session_renderable_shows_curse_bonus_reward_and_cost_separately() -> (
    None
):
    session = start_new_game_session(seed=5, preferred_character_id="ironclad")
    provider = StarterContentProvider(session.content_root)
